...
import es.um.redes.nanoFiles.udp.message.MapSerializer;
...
public class NFControllerLogicDir {
	...
	public DirectoryConnector getDirectoryConnector() {
	...
	protected void getAndPrintFileList() {
		 ...
		 * operación.
		 */
		Map<FileInfo, Set<InetSocketAddress>> trackedFiles = directoryConnector.getFileList();
		...
		FileInfo.printToSysoutWithServers(trackedFiles);
			...
			System.out.println("* There are no files tracked by the directory at " + directoryConnector.getDirectoryHostname());
		}

	}

	/**
	 * Método para obtener y mostrar la lista de usuarios que
	 * hay conectados al directorio, si han compartido o no, con que IP y que ficheros.
	 */
	protected void getAndPrintUserList() {
		Map<InetSocketAddress, Map<InetSocketAddress, Set<String>>> trackedUsers = directoryConnector.getUserList();
		if(trackedUsers!=null && !trackedUsers.isEmpty()){
			System.out.println("* These are the users connected to the directory: ");
			MapSerializer.printMapAsTable(trackedUsers);
		}else{
