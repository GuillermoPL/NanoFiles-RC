...
public class NFController {
	...
	public NFController(String defaultDirectory) {
		...
		boolean pingSuccess = controllerDir.ping();
		if (pingSuccess) {
   			currentState = ONLINE;
   			System.out.println("Connected successfully! Now ONLINE.");
		} else {
    		System.err.println("Failed to connect to directory. Still OFFLINE.");
		}

	}
	public NFControllerLogicDir getNFControllerLogicDir(){
	...
	public void processCommand() {
		...
		case NFCommands.COM_USERLIST:
			/*
			 * Pedir al controllerDir que obtenga del directorio la lista de usuarios que
			 * están conectados al directorio, determinando si están sirviendo, con que IP y que ficheros, y la imprima.
			 * por pantalla (método getAndPrintUserList)
			 */
			controllerDir.getAndPrintUserList();
			break;
		case NFCommands.COM_SERVE:
			...
			if (NanoFiles.testModeTCP) {
				...
				if(NanoFiles.db.getFiles().length > 0) {
					if (puertoServe < 0 || puertoServe > 65535) {
           				 System.err.println("Error: el puerto debe estar entre 0 y 65535.");
						 break;
        			}if(puertoServe==0){
						System.err.println("Elige otro puerto.");
						break;
					}
					boolean serverRunning = controllerPeer.startFileServer(puertoServe);
					if (serverRunning) {
					...
					System.err.println("There are no files available to publish");
				}
		...
		case NFCommands.COM_DOWNLOAD:
			...
			if (NanoFiles.testModeTCP) {
				...
				InetSocketAddress[] serverAddressList = controllerDir.getServerAddressesSharingThisFile(targetFilenameSubstring);
				if(serverAddressList!=null && serverAddressList.length!=0){
					for(InetSocketAddress dir : serverAddressList){
						System.out.println(dir);
					}
					commandSucceeded = controllerPeer.downloadFileFromServers(serverAddressList, targetFilenameSubstring, downloadLocalFileName, chunkSize);
				}else if(serverAddressList==null){
					System.out.println("No hay coincidencias de ficheros cuyo nombre contenga la subcadena: " + targetFilenameSubstring);
				}else{
					System.out.println("Ambiguedad encontrada, varios ficheros contienen la subcadena: " + targetFilenameSubstring);
				}
			}
			break;
		...
		case NFCommands.COM_UPLOAD:
			...
			if (matchingFiles.length == 1) {
				commandSucceeded = controllerPeer.uploadFileToServer(matchingFiles[0], uploadToServer, chunkSize);
	...
	private boolean canProcessCommandInCurrentState() { 
		...
		case NFCommands.COM_PING: {
			commandAllowed = currentState == ONLINE || currentState == SERVING || currentState == OFFLINE;
			break;
		}
		case NFCommands.COM_USERLIST: {
	...
	private void updateCurrentState(boolean success) {
		...
		switch (currentCommand) {
		//Notemos que ping no nos actualzia a ONLINE pues perderiamos el estado SERVING, solo lo hace en el constructor.
		case NFCommands.COM_SERVE:
	...
	private void setCurrentCommandArguments(String[] args) {
		...
		case NFCommands.COM_DOWNLOAD:
			...
			try {
			    ...
			    System.err.println("Error: el argumento no es un número válido.");
			}
			break;
		case NFCommands.COM_UPLOAD:
			targetFilenameSubstring = args[0];
		...
		case NFCommands.COM_SERVE:
			try {
			    puertoServe = Integer.parseInt(args[0]);
			} catch (NumberFormatException e) {
			    System.err.println("Error: el argumento no es un número válido.");
			}
			break;
