...
public class NFShell {
	...
	private boolean validateCommandArguments(String[] args) {
		...
		case NFCommands.COM_DOWNLOAD:
			if (args.length != 3) {
				System.out.println(
						"Correct use:" + NFCommands.commandToString(command) + " <filename_substring> <local_filename> <chunk_size>");
				return false;
			}
			break;
		case NFCommands.COM_UPLOAD:
			if (args.length != 3) {
				System.out.println(
						"Correct use:" + NFCommands.commandToString(command) + " <filename_substring> <remote_server> <chunk_size>");
				return false;
			}
			break;
		case NFCommands.COM_SERVE:
